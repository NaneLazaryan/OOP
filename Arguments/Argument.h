...
public:
	...
	// Constructors
	Argument(const std::string& name, ArgType type) 
		: name(name), type(type), value(std::monostate{}), flag(false) {}
	
	Argument(const std::string& name, ArgType type, const ArgValue& value) 
		: name(name), type(type), value(value), flag(true) {}
